...
function addAttrValues() {
    ...
    var subCat = $('#catname').val();
    if(vals!==-1)
    {    
        var avals = $('#catSelect option:selected').text();
        var txtid = vals;
        if (tagArray.indexOf(txtid) == -1 && subCat.indexOf(avals) == -1)
        {
        ...
        else if(subCat.indexOf(avals) !== -1)
        {
            common.toast(0, "Parent category and sub category cannot be same");
            $('#catSelect').val('0');
        }
        else 
        {
