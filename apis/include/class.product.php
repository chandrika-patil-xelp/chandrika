	...
	class product extends DB
    {
		...
		if(!$cnt1)
		{
                ...
                $sql  = "SELECT
                         ...
                         WHERE
                                    product_name = \"".$detls['product_name']."\"
                         AND 
...
// Uploading the image method.        
       /* public function imageUpdate($params)
        ...
        { 
            ...
            $sql = "SELECT *,
                                MATCH(product_name) AGAINST ('".$params['prname']."*' IN BOOLEAN MODE) as startwith 
                    FROM 
                                tbl_product_master
                    WHERE
                                MATCH(product_name) AGAINST ('".$params['prname']."*' IN BOOLEAN MODE)
                    ORDER BY
                                startwith DESC";
            if (!empty($page))
            {
          ...
          $limit  = ($params['limit'] ? $params['limit'] : 15);
            ...
            if ($res)
            {
                ...
                $sql4="SELECT 
                                ...
                                vatno as Vat_Number,                            
                                landline,
                                mdbw as other_diamond_around_world,                            
                                website,
                                banker as bankers,                            
                                pancard,
                                turnover,
        ...
        {
            $sql = "SELECT 
                                    *,IF(product_name LIKE '".$str."%',1,0) as startwith 
                    FROM
                                    $tblname 
                    WHERE
                                    MATCH(product_name) AGAINST (\"'" . $str . "*'\" IN BOOLEAN MODE)
                    ORDER BY
                                    startwith 
                    DESC LIMIT 0,15";
            $res=$this->query($sql);
