...
class category extends DB
{
    ...
    {
        $err=array('err_code'=>1,'err_msg'=>'Something Went Wrong');
        $results=array('result'=>array(),'error'=>$err);

        $sql = "SELECT * from tbl_category_master where active_flag = 1 and pcatid = ".$cid." ORDER BY pcatid ASC, cat_name ASC";
        //echo "<br>";
        $res = $this->query($sql);
        if($res)
        {
            ...
            {
                $data[$row['pcatid']] = $row;
                $val = $this->getCatgoryTree($row['catid']);
                $data[$row['pcatid']]['child'] = $val;
            }
        }
        //$results=array('result'=>$data,'error'=>$err);
        return $data;
    }
