...
class viewlog extends DB
{
    ...
    {  
        ...
        $udsql="select logmobile,userName,email from tbl_registration where logmobile=".$params['mobile']."";
        ...
        $isql="INSERT INTO tbl_viewlog(umob,userName,email,product_id,vendormobile,updatedby,udt,cdt)
               VALUES(".$udetail[$j]['logmobile'].",'".$udetail[$j]['userName']."','".$udetail[$j]['email']."',".$prod[$i]['product_id'].",".$prod[$i]['vendormobile'].",'customer',now(),now())";
        ...
        {
            $arr="There is no product registered with this vendor";
    ...
    public function viewLog($params)
    {
       ...
       $viewprod="SELECT umob,userName,email,cdt,product_id,vendormobile from tbl_viewlog where vendormobile=".$params['logmobile'];
