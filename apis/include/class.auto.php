...
class auto extends DB
{
    ...
    {
	$sql="SELECT product_id,product_display_name,product_model,product_brand,category_id,brand_id FROM tbl_product_master where product_name LIKE '%".$params['srch']."%' ORDER BY product_name DESC LIMIT 0,10";
        $res=$this->query($sql);
        if($this->numRows($res))
        {
