   ...
   var filter = /^[0-9-+]+$/;
    ...
    if (validationFlag == 1)
    {
      ...
      $.ajax({  url: URL, type: "GET", datatype: "JSON",  success: function(res)  {
			 ...
			 common.msg(0,'Email id is already registered');
		       }
	  });
 ...
 function checkuser()
 {
    ...
    var URL= APIDOMAIN + "index.php?action=getUserdetailbymob&mob="+inptval;
      ...
      $.ajax({  url: URL,
			 ...
			 if($.isNumeric(inptval))
                             common.msg(0,'Mobile No. not registered');
			 else
                             common.msg(0,'Email-id not registered');
		       }
		    }
      });
 }
 ...
 function restpasssubmt()
 {
   ...
   if(pass ===''|| pass === null){
       common.msg(0,'Please enter your Password');
        ...
        common.msg(0,'Please confirm your password');
        validationFlag=0;
        return false;
    }
    if(validationFlag == 1){
    if(pass !== cpass){
      common.msg(0,'Please enter the same password');
    }
    else{
   ...
   function checkotp(otpval)
  {
	...
	var URL= APIDOMAIN + "index.php?action=checkopt&mobile="+mailmob+"&otpval="+otpval;
      $.ajax({  url: URL, type: "GET",  datatype: "JSON", success: function(results) {
		      ...
		      var data=obj.result;
		      if(data.otp==null){
			common.msg(0,'OTP expired please try again.');
		      }
		      else{
		      if(otpval==data.otp){
		if(otpflg == 1)
  ...
  function sendemailotp(inptval)
  {
    ...
    $.ajax({type:"POST", url: URL, success:function(res){
	 common.msg(1,'OTP has sent to your email-id');
	 var data=JSON.parse(res);
	 mailmob=data['mob'];
    }
    });
